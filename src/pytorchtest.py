...
train_data = []
waited = False
files = listdir('./allebilder/')
for i in range(len(listdir('./allebilder/'))):
    if len(train_data) == 58 and not waited:
        waited = True
        continue
    f = random.choice(files)
    files.remove(f)
    img = Image.open("./allebilder/" + f)
    img_tensor = transforms(img)#(3,256,256)
    train_data_list.append(img_tensor)
    isCat = 1 if 'background' in f else 0
    isDog = 1 if 'luca' in f else 0
    target = [isCat, isDog]
    target_list.append(target)
    if len(train_data_list) >= 32:
        train_data.append((torch.stack(train_data_list), target_list))
        train_data_list = []
        target_list = []
        print('Loaded batch ', len(train_data), 'of ', int(len(listdir('./allebilder/'))/64))
        print('Percentage Done: ', 100*len(train_data)/int(len(listdir('./allebilder/'))/64), '%')
