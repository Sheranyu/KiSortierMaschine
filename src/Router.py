...
class Router:
    ...
    def register_route(self) -> None:
        self.routes = {
            ...
            "/create-model": ft.View("/create-model", controls=CreateModelPage(self.page).build(),
                appbar=ft.AppBar(title=ft.Text("Modell erstellen"), bgcolor=ft.colors.SURFACE_VARIANT)),
            "/load-model": ft.View("/load-model", controls=LoadModelPage(self.page).build(), 
                appbar=ft.AppBar(title=ft.Text("Modell laden"), bgcolor=ft.colors.SURFACE_VARIANT)),
            "/start-application": ft.View("/start-application", controls=StartApplicationPage(self.page).build(), 
